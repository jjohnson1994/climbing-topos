...
import { crags, globals } from "@/app/api";
...
function CreateCrag() {
  ...
  const [cragLocationLoading, setCragLocationLoading] = useState<boolean>(
    false,
  );
  ...
  const formOnSubmit = handleSubmit(async (formData) => {
    try {
      ...
      const { slug } = await crags.createCrag({ ...formData, osmData, image });
