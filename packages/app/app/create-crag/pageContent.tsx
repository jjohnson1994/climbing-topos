...
import { reverseLookup } from '@/app/helpers/nominatim';
import AuthenticatedRoute from '@/app/components/AuthenticatedRoute';
import { objectToFormData } from '../helpers/api';

const schema = NewCragSchema();

let image: File;

function CreateCrag({ post }) {
  ...
  const formOnSubmit = handleSubmit(async (formData) => {
    try {
      ...
      formDataObject.set('latitude', formData.latitude);
      formDataObject.set('tags', JSON.stringify(formData.tags));
      formDataObject.set('description', formData.description);
      formDataObject.set('title', formData.title);
