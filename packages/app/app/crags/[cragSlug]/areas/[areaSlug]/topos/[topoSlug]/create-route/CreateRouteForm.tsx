...
import { get as getArea } from '@/app/data/actions/areas/get';
import { get as getTopo } from '@/app/data/actions/topos/get';
import { useRouter } from 'next/navigation';
import TopoCanvas from '@/app/components/TopoCanvas';
...
function CreateRouteForm({
  ...
  useEffect(() => {
    ...
    const doGetTopo = async () => {
      const topo = await getTopo(topoSlug);
  ...
  const formOnSubmit = handleSubmit(async (formData) => {
    try {
      ...
      router.push(`/crags/${cragSlug}/areas/${areaSlug}#${routeSlug}`);
    } catch (error) {
