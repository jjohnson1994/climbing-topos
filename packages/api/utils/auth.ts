...
const getUserAttributes = async (event: APIGatewayProxyEventV2) => {
  ...
  const userPoolId = authorizer.iam.cognitoIdentity.amr
    .find((amr: string) => amr.includes('CognitoSignIn'))
    .split(':')
    .shift()
    .split('/')
    .pop();
  const userPoolUserId = authorizer.iam.cognitoIdentity.amr
    .find((amr: string) => amr.includes('CognitoSignIn'))
    .split(':')
    .pop();

  const { Users: users } = await cognito.send(
